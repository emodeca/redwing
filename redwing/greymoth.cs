...
namespace redwing
{
    internal class greymoth : MonoBehaviour
    {
        ...
        private int antiTurboLeft;

        private bool completedCoroutines;
        public Vector2 DashDirection;
        private float dashInvulTimer;
        ...
        {
            return hasSharpShadowCached ? HeroController.instance.DASH_SPEED_SHARP : HeroController.instance.DASH_SPEED;
        }
        ...
        private void Update()
        {
            ...
            if (dashInvulTimer > 0) dashInvulTimer -= Time.deltaTime;
        ...
        private bool dashTapped()
        {
            ...
            if (sharpShadowFSM != null) sharpShadowFSM.SetState("Idle");
        ...
        private void doDash()
        {
            ...
            if ((!HeroController.instance.cState.onGround || Math.Abs(DashDirection.y) > 0.001f) &&
                !HeroController.instance.inAcid) getPrivateField("airDashed").SetValue(HeroController.instance, true);

            invokePrivateMethod("ResetAttacksDash");
            invokePrivateMethod("CancelBounce");
            ((HeroAudioController) getPrivateField("audioCtrl").GetValue(HeroController.instance)).StopSound(
                HeroSounds.FOOTSTEPS_RUN);
            ((HeroAudioController) getPrivateField("audioCtrl").GetValue(HeroController.instance)).StopSound(
                HeroSounds.FOOTSTEPS_WALK);
            ((HeroAudioController) getPrivateField("audioCtrl").GetValue(HeroController.instance)).StopSound(
                HeroSounds.DASH);
            ...
            if (HeroController.instance.cState.wallSliding)
            {
                ...
                if (HeroController.instance.cState.facingRight && Math.Abs(DashDirection.y) < 0.001f)
                    DashDirection.x = 1f;
                else if (Math.Abs(DashDirection.y) < 0.001f) DashDirection.x = -1f;
            }
            ...
            if (HeroController.instance.cState.shadowDashing)
            {
                ...
                {
                    getPrivateField("dashEffect").SetValue(HeroController.instance,
                        HeroController.instance.shadowdashBurstPrefab.Spawn(new Vector3(
                            HeroController.instance.transform.position.x + 5.21f,
                            HeroController.instance.transform.position.y - 0.58f,
                            HeroController.instance.transform.position.z + 0.00101f)));
                    ((GameObject) getPrivateField("dashEffect").GetValue(HeroController.instance)).transform.localScale
                        = new Vector3(1.919591f,
                            ((GameObject) getPrivateField("dashEffect").GetValue(HeroController.instance)).transform
                            .localScale.y,
                            ((GameObject) getPrivateField("dashEffect").GetValue(HeroController.instance)).transform
                            .localScale.z);
                }
                ...
                FSMUtility.LocateFSM(HeroController.instance.shadowRechargePrefab, "Recharge Effect")
                    .SendEvent("RESET");
                var ps = HeroController.instance.shadowdashParticlesPrefab.GetComponent<ParticleSystem>();
                var em = ps.emission;
                em.enabled = true;
            ...
            else
            {
                ...
                var ps = HeroController.instance.dashParticlesPrefab.GetComponent<ParticleSystem>();
                var em = ps.emission;
                em.enabled = true;
            }
            ...
            {
                getPrivateField("dashEffect").SetValue(HeroController.instance,
                    HeroController.instance.backDashPrefab.Spawn(HeroController.instance.transform.position));
                ((GameObject) getPrivateField("dashEffect").GetValue(HeroController.instance)).transform.localScale =
                    new Vector3(HeroController.instance.transform.localScale.x * -1f,
                        HeroController.instance.transform.localScale.y, HeroController.instance.transform.localScale.z);
            }
        ...
        {
            var didAirDash = (bool) getPrivateField("airDashed").GetValue(HeroController.instance);
