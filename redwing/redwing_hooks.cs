...
namespace redwing
{
    [Obsolete("This is deprecated. Switch to rebalanced_hooks")]
    public class redwing_hooks : MonoBehaviour
    {
        ...
        {
            if (fsCharge >= 0.0 && othercollider.gameObject.layer == 11) fsCharge -= fsReduceOnHit;
        }

        private bool setTrailCooldown()
        {
            if (ftTime <= 0.0) useFT = true;
        ...
        {
            return Blackmoth.Instance.grubberOn;
        }
        ...
        private void createFlameShield()
        {
            ...
            var localPosition = flameShieldObj.transform.localPosition;
            localPosition = Vector3.zero;
            localPosition = new Vector3(0f, -0.4f);
            flameShieldObj.transform.localPosition = localPosition;
        ...
        {
            if (fsCharge > 0.0) fsCharge -= Time.deltaTime;

            if (laserTime > 0.0) laserTime -= Time.deltaTime;

            if (fbTime > 0.0) fbTime -= Time.deltaTime;

            if (invulnTime > 0.0) invulnTime -= Time.deltaTime;

            if (ftTime > 0.0) ftTime -= Time.deltaTime;

            if (cycloneTime > 0.0) cycloneTime -= Time.deltaTime;
        }
        ...
        public void cycloneSlashFireballs()
        {
            ...
            if (cycloneTime <= 0.0) StartCoroutine(randomCycloneBalls());
        ...
        private IEnumerator randomCycloneBalls()
        {
            ...
            var timePassed = 0f;
            var maxTime = HeroController.instance.INVUL_TIME_CYCLONE * 2;
