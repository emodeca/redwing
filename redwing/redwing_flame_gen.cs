...
namespace redwing
{
    internal class redwing_flame_gen : MonoBehaviour
    {
        ...
        private void generateSoundEffects()
        {
            ...
            fireballHitSoundFX = AudioClip.Create("fireball1", (int) (AUDIO_SAMPLE_HZ * 1.0) + 30000, 1,
                AUDIO_SAMPLE_HZ, false);
            ...
            shieldRechargeSoundFX =
                AudioClip.Create("shieldRecharge", (int) (AUDIO_SAMPLE_HZ * 0.7), 1, AUDIO_SAMPLE_HZ, false);
            shieldRechargeSoundFX.SetData(generateShieldChargeSound((int) (AUDIO_SAMPLE_HZ * 0.7)), 0);

            shieldDischargeSoundFX =
                AudioClip.Create("shieldRecharge", (int) (AUDIO_SAMPLE_HZ * 0.4), 1, AUDIO_SAMPLE_HZ, false);
            shieldDischargeSoundFX.SetData(generateShieldDischargeSound((int) (AUDIO_SAMPLE_HZ * 0.4)), 0);

            load_textures.nukeSound = AudioClip.Create("nukeBeep", AUDIO_SAMPLE_HZ * 6, 1, AUDIO_SAMPLE_HZ, false);
            load_textures.nukeSound.SetData(generateNukeBeep(AUDIO_SAMPLE_HZ * 6), 0);
        }
        ...
        private float[] generatePinkNoise(float volume, float[] fx)
        {
            ...
            try
            {
                for (var i = 0; i < fx.Length; i++) fx[i] = generatePinkValue() * volume;
            }
            catch (Exception e)
            {
        ...
        private float generatePinkValue()
        {
            ...
            if (pinkIndex != 0)
            {
                ...
                newRandom = newRandom | rng.Next(int.MinValue, int.MaxValue);
            ...
            newRandom = newRandom << 32;
            newRandom = newRandom | rng.Next(int.MinValue, int.MaxValue);
            var sum = pinkRunningSum + newRandom;

            var f = pinkScalar * sum;
            if (f > 1.0)
                f = 1.0f;
        ...
        private float[] generateFbSound(float[] fx)
        {
            try
            {
                ...
                for (var i = 0; i < fx.Length; i++) fx[i] = fx[i] + fx2[i];
        ...
        {
            for (var i = 0; i < fx.Length; i++) fx[i] *= (float) volume;
            return fx;
        }

        private float[] speedUpEffect(double speed, float[] fx)
        {
            var speededFx = new float[fx.Length];

            for (var i = 0; i < fx.Length; i++)
            {
                var realPosition = i * speed;
                var positionRoundedDown = (int) realPosition;
                var weighting = realPosition % 1.0;
                if (positionRoundedDown + 1 < fx.Length)
                    speededFx[i] = (float) ((1.0 - weighting) * fx[positionRoundedDown] +
                                            weighting * fx[positionRoundedDown + 1]);
                else
                    break;
            }
        ...
        private float[] generateChirp(double volume, double startHz, double endHz, int startTime, int endTime,
            float[] fx)
        {
            var modulateTime = (endTime - startTime) / (AUDIO_SAMPLE_HZ * 0.5);
        ...
        private float[] generateSirenSound(double volume, double minHz, double maxHz, int startTime, int maxTime,
            double modulateTime,
            float[] fx)
        {
            ...
            double startingPoint, float[] fx)
        {
            ...
            var minHzForComparison = Math.Log(minHz, 2.0);
            var maxHzForComparison = Math.Log(maxHz, 2.0);
            var modulateInterval = modulateTime * AUDIO_SAMPLE_HZ;


            log("min freq is " + minHz);
            log("max freq is " + maxHz);

            for (var i = startTime; i < maxTime; i++)
            {
                double frequencyPos;
                if ((i - startTime + startingPoint * modulateInterval) % modulateInterval > modulateInterval / 2)
                    ...
                    frequencyPos = (i - startTime + startingPoint * modulateInterval) % (modulateInterval / 2) /
                                   (modulateInterval / 2);
                ...
                var freqForComparison =
                    (1.0f - frequencyPos) * minHzForComparison + frequencyPos * maxHzForComparison;
                var actualFreq = Math.Pow(2.0, freqForComparison);

                var sinePitchTime = AUDIO_SAMPLE_HZ / actualFreq / 2.0;

                //if (i % 500 == 0)
                //    log("sine pitch time is " + sinePitchTime + " because the freq is " + actualFreq);
                // Will this work? Who knows.
                fx[i] += (float) (volume * Math.Sin(randomPointOnSin + i / sinePitchTime));
            }
        ...
        {
            var slowedFx = new float[fx.Length];
            var positionRoundedDown = 0;
            ...
            for (var i = 0; i < fx.Length; i++)
            {
                ...
                var weighting = realPosition % 1.0;
                if (positionRoundedDown + 1 < fx.Length)
                    slowedFx[i] = (float) ((1.0 - weighting) * fx[positionRoundedDown] +
                                           weighting * fx[positionRoundedDown + 1]);
                else
                    break;
            }
        ...
        private float[] highPitchExplosionSound(double volume, float[] fx)
        {
            ...
            for (var i = 0; i < fx.Length; i++) fx[i] = fx[i] + pass1[i] + pass2[i] + pass3[i];
        ...
        private float[] fadeAudio(double fadeVolume, int startTime, int endFade, float[] fx)
        {
            try
            {
                ...
                for (var i = startTime; i < endFade; i++)
                {
                    var fadeAmount = (i - startTime) / (double) (endFade - startTime);
                    // weighted avg
                    var weightedVol = 1.0 - fadeAmount + fadeAmount * fadeVolume;
                    var actualVolume = (float) Math.Pow(weightedVol, 2.0);

                    fx[i] *= actualVolume;
                }

                // now fade everything else past the end.
                for (var i = endFade; i < fx.Length; i++) fx[i] *= (float) fadeVolume;
            }
            catch (Exception e)
            {
        ...
        private float[] generateWhiteNoise(double volume, int startTime, int endTime, float[] fx)
        {
            try
            {
                ...
                {
                    var r = rng.NextDouble() * 2.0 - 1.0;
                    fx[i] += (float) (r * volume);
                }
            }
            catch (Exception e)
            {
                log("Unable to generate white noise from time " + startTime + " to " + endTime +
                    " probably because out of bounds " + e);
            }
        ...
        private float[] generateSawtooth(double volume, double frequency, float[] fx, int startTime, int endTime)
        {
            ...
            var pitchTime = AUDIO_SAMPLE_HZ / frequency;

            // Don't generate every frequency at the same point
            // Gotta pick random ones.
            var randomOffset = rng.NextDouble() * pitchTime;

            var inverseData = false;
            var doInverse = rng.NextDouble();
            if (doInverse >= 0.5) inverseData = true;
            ...
            try
            {
                for (var i = startTime; i < endTime; i++)
                {
                    var currentPos = i + randomOffset;
                    var currentVal = 2 * (currentPos % pitchTime) / pitchTime;
                    if (inverseData) currentVal *= -1;
                    fx[i] += (float) (currentVal * volume);
                }
            }
            catch (Exception e)
            {
                log("Unable to generate sawtooth from time " + startTime + " to " + endTime +
                    " probably because out of bounds " + e);
            }
        ...
        {
            var realVolume = volume * freqInterval / (freqMax - freqMin + freqInterval);
            for (var i = freqMin; i <= freqMax; i += freqInterval)
            {
                randomPointOnSin = rng.NextDouble() * 2.0 * Math.PI;
        ...
        {
            // get number of samples that a pitch lasts
            var pitchTime = AUDIO_SAMPLE_HZ / freq;

            // Then divide this number by 2 pi because sine formula
            // lasts that long
            var sinePitchTime = pitchTime / (2.0 * Math.PI);
            ...
            {
                for (var i = startTime; i < endTime; i++)
                    fx[i] += (float) (volume * Math.Sin((i + randomPointOnSin) / sinePitchTime));
            }
            catch (Exception e)
            {
                log("Unable to generate noise from time " + startTime + " to " + endTime +
                    " probably because out of bounds " + e);
            }
        ...
        private float[] normalizeVolume(float[] fx)
        {
            ...
            if (maxVolume >= 1.0)
            {
                for (var i = 0; i < fx.Length; i++) fx[i] /= maxVolume;
        ...
        private float[] audioClipping(float maxVol, float[] fx)
        {
            if (maxVol > 1.0f)
                maxVol = 1.0f;
            else if (maxVol < 0.0f) maxVol = 0.0f;
            for (var i = 0; i < fx.Length; i++)
                if (fx[i] > maxVol)
                    fx[i] = maxVol;
                else if (fx[i] < -maxVol) fx[i] = -maxVol;
            return fx;
        }
        ...
        private void generateFlameTextures()
        {
            ...
            try
            {
                for (var i = 0; i < firePillars.Length; i++)
                {
            ...
            try
            {
                for (var i = 0; i < fireSpikes.Length; i++)
                {
            ...
            try
            {
                for (var i = 0; i < fireballMagmas.Length; i++)
                {
            ...
            try
            {
                for (var i = 0; i < fireballMagmaBalls.Length; i++)
                {
            ...
            try
            {
                for (var i = 0; i < fireShields.Length; i++)
                {
            ...
            var fireTrailMiddles = new Texture2D[4];
            var fireTrailCaps = new Texture2D[4];
            var reverseFireTrailCaps = new Texture2D[4];
            try
            {
                for (var i = 0; i < fireTrails.Length; i++)
                {
                    ...
                    for (var x = 0; x < reverseFireTrailCaps[i].width; x++)
                    for (var y = 0; y < reverseFireTrailCaps[i].height; y++)
                        fireTrails[i].SetPixel(x, y, reverseFireTrailCaps[i].GetPixel(x, y));

                    var offset = reverseFireTrailCaps[i].width;

                    for (var x = 0; x < fireTrailMiddles[i].width; x++)
                    for (var y = 0; y < fireTrailMiddles[i].height; y++)
                        fireTrails[i].SetPixel(x + offset, y, fireTrailMiddles[i].GetPixel(x, y));

                    offset += fireTrailMiddles[i].width;

                    for (var x = 0; x < fireTrailCaps[i].width; x++)
                    for (var y = 0; y < fireTrailCaps[i].height; y++)
        ...
        {
            var circle = new Texture2D(100, 100);
            for (var x = 0; x < 100; x++)
            for (var y = 0; y < 100; y++)
            {
                var dist = getDistance(x, y, 100, 100);
                if (dist < 35.0)
        ...
        {
            var meme = generateFireTrailCap();
            var newMeme = new Texture2D(FTCAPTEXTURE_WIDTH, FTTEXTURE_HEIGHT);

            // Just reverse it manually because computers are fast and who cares.
            for (var x = 0; x < FTCAPTEXTURE_WIDTH; x++)
            for (var y = 0; y < FTTEXTURE_HEIGHT; y++)
        ...
        {
            var ftc = new Texture2D(FTCAPTEXTURE_WIDTH, FTTEXTURE_HEIGHT);
            var horzIntensity150 = new double[FTTEXTURE_HEIGHT];
            var horzOpacity150 = new double[FTTEXTURE_HEIGHT];

            for (var i = 0; i < FTTEXTURE_HEIGHT; i++)
            {
            ...
            // Interpolation
            for (var i = 0; i < FTTEXTURE_HEIGHT - FT_CAP_INTERPOLATE_PIXELS; i++)
            {
                if (i % FT_CAP_INTERPOLATE_PIXELS == 0) continue;
                var offset = i % FT_CAP_INTERPOLATE_PIXELS;
                var avgWeighting = offset / (double) FT_CAP_INTERPOLATE_PIXELS;

                horzIntensity150[i] = horzIntensity150[i - offset + FT_CAP_INTERPOLATE_PIXELS] * avgWeighting +
                                      horzIntensity150[i - offset] * (1.0 - avgWeighting);
                horzOpacity150[i] = horzOpacity150[i - offset + FT_CAP_INTERPOLATE_PIXELS] * avgWeighting +
                                    horzOpacity150[i - offset] * (1.0 - avgWeighting);
            }

            // Interpolation phase pt 2 (for wrap around)
            for (var i = FTTEXTURE_HEIGHT - FT_CAP_INTERPOLATE_PIXELS; i < FTTEXTURE_HEIGHT; i++)
            {
                if (i % FT_CAP_INTERPOLATE_PIXELS == 0) continue;
                var offset = i % FT_CAP_INTERPOLATE_PIXELS;
                var avgWeighting = offset / (double) FT_CAP_INTERPOLATE_PIXELS;

                horzIntensity150[i] = horzIntensity150[0] * avgWeighting +
                                      horzIntensity150[i - offset] * (1.0 - avgWeighting);
                horzOpacity150[i] =
                    horzOpacity150[0] * avgWeighting + horzOpacity150[i - offset] * (1.0 - avgWeighting);
            }
            ...
            {
                var distance =
                    Math.Sqrt(1.5 * (x * x) + 1.0 * ((y - FTTEXTURE_HEIGHT / 2) * (y - FTTEXTURE_HEIGHT / 2)));
        ...
        {
            var fbm = new Texture2D(FBTEXTURE_WIDTH, FBTEXTURE_HEIGHT);
            var vertIntensity150 = new double[FBTEXTURE_WIDTH];
            var vertOpacity150 = new double[FBTEXTURE_WIDTH];

            // RNG phase
            for (var i = 0; i < FBTEXTURE_WIDTH; i++)
            {
            ...
            for (var i = 0; i < FBTEXTURE_WIDTH - FB_MAGMA_INTERPOLATE_PIXELS; i++)
            {
                if (i % FB_MAGMA_INTERPOLATE_PIXELS == 0) continue;
                var offset = i % FB_MAGMA_INTERPOLATE_PIXELS;
                var avgWeighting = offset / (double) FB_MAGMA_INTERPOLATE_PIXELS;

                vertIntensity150[i] = vertIntensity150[i - offset + FB_MAGMA_INTERPOLATE_PIXELS] * avgWeighting +
                                      vertIntensity150[i - offset] * (1.0 - avgWeighting);
                vertOpacity150[i] = vertOpacity150[i - offset + FB_MAGMA_INTERPOLATE_PIXELS] * avgWeighting +
                                    vertOpacity150[i - offset] * (1.0 - avgWeighting);
            }

            // Interpolation phase pt 2 (for wrap around)
            for (var i = FBTEXTURE_WIDTH - FB_MAGMA_INTERPOLATE_PIXELS; i < FBTEXTURE_WIDTH; i++)
            {
                if (i % FB_MAGMA_INTERPOLATE_PIXELS == 0) continue;
                var offset = i % FB_MAGMA_INTERPOLATE_PIXELS;
                var avgWeighting = offset / (double) FB_MAGMA_INTERPOLATE_PIXELS;

                vertIntensity150[i] = vertIntensity150[0] * avgWeighting +
                                      vertIntensity150[i - offset] * (1.0 - avgWeighting);
                vertOpacity150[i] =
                    vertOpacity150[0] * avgWeighting + vertOpacity150[i - offset] * (1.0 - avgWeighting);
            }

            // Actually set the colors
            for (var x = 0; x < FBTEXTURE_WIDTH; x++)
            {
                var xDistance = x - FBTEXTURE_WIDTH / 2;
                if (xDistance < 0) xDistance = -xDistance;

                xDistance = (int) (xDistance * (2.5 - 2.0 * index / 12.0));

                for (var y = 0; y < FBTEXTURE_HEIGHT; y++)
                {
                    var netDistance = (int) Math.Sqrt(xDistance * xDistance + y * y);
        ...
        {
            var fs = new Texture2D(LASERTEXTURE_WIDTH, LASERTEXTURE_HEIGHT);
            var horzIntensity20 = new double[LASERTEXTURE_HEIGHT];
            var horzOpacity20 = new double[LASERTEXTURE_HEIGHT];


            // RNG phase
            for (var i = 0; i < LASERTEXTURE_HEIGHT; i++)
            {
            ...
            // Interpolation phase
            for (var i = 0; i < LASERTEXTURE_HEIGHT - FS_INTERPOLATE_PIXELS; i++)
            {
                if (i % FS_INTERPOLATE_PIXELS == 0) continue;
                var offset = i % FS_INTERPOLATE_PIXELS;
                var avgWeighting = offset / (double) FS_INTERPOLATE_PIXELS;

                horzIntensity20[i] = horzIntensity20[i - offset + FS_INTERPOLATE_PIXELS] * avgWeighting +
                                     horzIntensity20[i - offset] * (1.0 - avgWeighting);
                horzOpacity20[i] = horzOpacity20[i - offset + FS_INTERPOLATE_PIXELS] * avgWeighting +
                                   horzOpacity20[i - offset] * (1.0 - avgWeighting);
            }

            // Interpolation phase pt 2 (for wrap around)
            for (var i = LASERTEXTURE_HEIGHT - FS_INTERPOLATE_PIXELS; i < LASERTEXTURE_HEIGHT; i++)
            {
                if (i % FS_INTERPOLATE_PIXELS == 0) continue;
                var offset = i % FS_INTERPOLATE_PIXELS;
                var avgWeighting = offset / (double) FS_INTERPOLATE_PIXELS;

                horzIntensity20[i] = horzIntensity20[0] * avgWeighting +
                                     horzIntensity20[i - offset] * (1.0 - avgWeighting);
            ...
            // Actually set the colors
            for (var x = 0; x < LASERTEXTURE_WIDTH; x++)
            {
                var realDistance = LASERTEXTURE_WIDTH / 2.0 - 0.5 - x;

                for (var y = 0; y < LASERTEXTURE_HEIGHT; y++)
                    if (y > 50)
                        ...
                        fs.SetPixel(x, y, getFireColor
                        ((int) (y / 50.0 * realDistance), horzIntensity20[y], horzOpacity20[y], LASERTEXTURE_WIDTH / 2,
                            LASERTEXTURE_WIDTH / 2.0, 3.4, 9.0));
            }
        ...
        {
            var fp = new Texture2D(FPTEXTURE_WIDTH, FPTEXTURE_HEIGHT);
            var horzIntensity150 = new double[FPTEXTURE_HEIGHT];
            var horzOpacity150 = new double[FPTEXTURE_HEIGHT];


            // RNG phase
            for (var i = 0; i < FPTEXTURE_HEIGHT; i++)
            {
            ...
            // Interpolation phase
            for (var i = 0; i < FPTEXTURE_HEIGHT - FP_INTERPOLATE_PIXELS; i++)
            {
                if (i % FP_INTERPOLATE_PIXELS == 0) continue;
                var offset = i % FP_INTERPOLATE_PIXELS;
                var avgWeighting = offset / (double) FP_INTERPOLATE_PIXELS;

                horzIntensity150[i] = horzIntensity150[i - offset + FP_INTERPOLATE_PIXELS] * avgWeighting +
                                      horzIntensity150[i - offset] * (1.0 - avgWeighting);
                horzOpacity150[i] = horzOpacity150[i - offset + FP_INTERPOLATE_PIXELS] * avgWeighting +
                                    horzOpacity150[i - offset] * (1.0 - avgWeighting);
            }

            // Interpolation phase pt 2 (for wrap around)
            for (var i = FPTEXTURE_HEIGHT - FP_INTERPOLATE_PIXELS; i < FPTEXTURE_HEIGHT; i++)
            {
                if (i % FP_INTERPOLATE_PIXELS == 0) continue;
                var offset = i % FP_INTERPOLATE_PIXELS;
                var avgWeighting = offset / (double) FP_INTERPOLATE_PIXELS;
            ...
            // Actually set the colors
            for (var x = 0; x < FPTEXTURE_WIDTH; x++)
            for (var y = 0; y < FPTEXTURE_HEIGHT; y++)
        ...
        {
            var ft = new Texture2D(FTTEXTURE_WIDTH, FTTEXTURE_HEIGHT);
            var verticalIntensity150 = new double[FTTEXTURE_WIDTH];
            var verticalOpacity150 = new double[FTTEXTURE_WIDTH];

            // RNG phase
            for (var i = 0; i < FTTEXTURE_WIDTH; i++)
            {
            ...
            // Interpolation phase
            for (var i = 0; i < FTTEXTURE_WIDTH - FT_INTERPOLATE_PIXELS; i++)
            {
                if (i % FT_INTERPOLATE_PIXELS == 0) continue;
                var offset = i % FT_INTERPOLATE_PIXELS;
                var avgWeighting = offset / (double) FT_INTERPOLATE_PIXELS;

                verticalIntensity150[i] = verticalIntensity150[i - offset + FT_INTERPOLATE_PIXELS] * avgWeighting +
                                          verticalIntensity150[i - offset] * (1.0 - avgWeighting);
                verticalOpacity150[i] = verticalOpacity150[i - offset + FT_INTERPOLATE_PIXELS] * avgWeighting +
                                        verticalOpacity150[i - offset] * (1.0 - avgWeighting);
            }

            // Interpolation phase pt 2 (for wrap around)
            for (var i = FTTEXTURE_WIDTH - FT_INTERPOLATE_PIXELS; i < FTTEXTURE_WIDTH; i++)
            {
                if (i % FT_INTERPOLATE_PIXELS == 0) continue;
                var offset = i % FT_INTERPOLATE_PIXELS;
                var avgWeighting = offset / (double) FT_INTERPOLATE_PIXELS;

                verticalIntensity150[i] = verticalIntensity150[0] * avgWeighting +
                                          verticalIntensity150[i - offset] * (1.0 - avgWeighting);
                verticalOpacity150[i] = verticalOpacity150[0] * avgWeighting +
                                        verticalOpacity150[i - offset] * (1.0 - avgWeighting);
            }


            // Actually set the colors
            for (var x = 0; x < FTTEXTURE_WIDTH; x++)
            for (var y = 0; y < FTTEXTURE_HEIGHT; y++)
        ...
        {
            var fb = new Texture2D(FBMBTEXTURE_WIDTH, FBMBTEXTURE_HEIGHT);
            var radialIntensity400 = new double[360];
            var radialOpacity400 = new double[360];

            // Generation
            for (var i = 0; i < 360 - INTERPOLATE_DEGREES; i++)
            {
            ...
            {
                if (i % INTERPOLATE_DEGREES == 0) continue;
                var offset = i % INTERPOLATE_DEGREES;
                var avgWeighting = offset / (double) INTERPOLATE_DEGREES;

                radialIntensity400[i] = radialIntensity400[i - offset + INTERPOLATE_DEGREES] * avgWeighting +
                                        radialIntensity400[i - offset] * (1.0 - avgWeighting);
                radialOpacity400[i] = radialOpacity400[i - offset + INTERPOLATE_DEGREES] * avgWeighting +
                                      radialOpacity400[i - offset] * (1.0 - avgWeighting);
            }

            // Interpolation (but it wraps around)
            for (var i = 360 - INTERPOLATE_DEGREES; i < 360; i++)
            {
                var offset = i % INTERPOLATE_DEGREES;
                var avgWeighting = offset / (double) INTERPOLATE_DEGREES;

                radialIntensity400[i] = radialIntensity400[0] * avgWeighting +
                                        radialIntensity400[i - offset] * (1.0 - avgWeighting);
                radialOpacity400[i] = radialOpacity400[0] * avgWeighting +
                                      radialOpacity400[i - offset] * (1.0 - avgWeighting);
            }

            // Actually set the colors
            for (var x = 0; x < FBMBTEXTURE_HEIGHT; x++)
            for (var y = 0; y < FBMBTEXTURE_WIDTH; y++)
            {
                var angel = (int) getNearestAngel(x, y, FBMBTEXTURE_WIDTH, FBMBTEXTURE_HEIGHT);
        ...
        {
            var fb = new Texture2D(FBTEXTURE_WIDTH, FBTEXTURE_HEIGHT);
            var radialIntensity400 = new double[360];
            var radialOpacity400 = new double[360];

            // Generation
            for (var i = 0; i < 361 - INTERPOLATE_DEGREES; i++)
            {
                if (i % INTERPOLATE_DEGREES != 0) continue;
                radialIntensity400[i] = rng.NextDouble();
                radialOpacity400[i] = rng.NextDouble();
            ...
            for (var y = 0; y < FBTEXTURE_HEIGHT; y++)
            {
                var angel = (int) getNearestAngel(x, y, FBTEXTURE_WIDTH, FBTEXTURE_HEIGHT);
                fb.SetPixel(x, y, getFireColor
        ...
        {
            var fShield = new Texture2D(FSHIELDTEXTURE_WIDTH, FSHIELDTEXTURE_HEIGHT);
            var radialIntensityEdge = new double[360];
            var radialOpacityEdge = new double[360];
            ...
            for (var i = 0; i < 360 - INTERPOLATE_DEGREES; i++)
            {
                ...
                radialIntensityEdge[i] = radialIntensityEdge[i - offset + INTERPOLATE_DEGREES] * avgWeighting +
                                         radialIntensityEdge[i - offset] * (1.0 - avgWeighting);
                radialOpacityEdge[i] = radialOpacityEdge[i - offset + INTERPOLATE_DEGREES] * avgWeighting +
                                       radialOpacityEdge[i - offset] * (1.0 - avgWeighting);
            }
            ...
            for (var i = 360 - INTERPOLATE_DEGREES; i < 360; i++)
            {
                ...
                radialIntensityEdge[i] = radialIntensityEdge[0] * avgWeighting +
                                         radialIntensityEdge[i - offset] * (1.0 - avgWeighting);
                radialOpacityEdge[i] = radialOpacityEdge[0] * avgWeighting +
                                       radialOpacityEdge[i - offset] * (1.0 - avgWeighting);
            }

            // Actually set the colors
            for (var x = 0; x < FSHIELDTEXTURE_WIDTH; x++)
            for (var y = 0; y < FSHIELDTEXTURE_HEIGHT; y++)
            {
                var dist = ovalDistance(x, y, FSHIELDTEXTURE_WIDTH, FSHIELDTEXTURE_HEIGHT);
                ...
                {
                    var angel = (int) getNearestAngel(x, y, FSHIELDTEXTURE_WIDTH, FSHIELDTEXTURE_HEIGHT);
        ...
        {
            var intensity = getRealIntensity(distance, intensity400, intensInterpolateDist, intensWighting);
            ...
            {
                var intensitySeparation = flameIntensityThresholds[1] - flameIntensityThresholds[0];
                var intens1 = (intensity - flameIntensityThresholds[0]) / intensitySeparation;
                var intens2 = 1.0 - intens1;
            ...
            {
                var intensitySeparation = flameIntensityThresholds[2] - flameIntensityThresholds[1];
                var intens1 = (intensity - flameIntensityThresholds[1]) / intensitySeparation;
                var intens2 = 1.0 - intens1;
            ...
            {
                var intensitySeparation = flameIntensityThresholds[3] - flameIntensityThresholds[2];
                var intens1 = (intensity - flameIntensityThresholds[2]) / intensitySeparation;
                var intens2 = 1.0 - intens1;
        ...
        private double getRealOpacity(double distance, double opacity400, double interpolateDistance,
            double opacSharpness)
        {
            if (distance < 0.0) distance = -distance;
            var averageWeighting = distance / interpolateDistance;
            var opactReal = opacity400 * averageWeighting + (1.0 - averageWeighting) * opacSharpness;
            if (opactReal < 0.0)
                opactReal = 0.0;
            else if (opactReal >= 1.0) opactReal = 1.0;
            ...
            double intensityWeighting)
        {
            if (distance < 0.0) distance = -distance;
            var averageWeighting = distance / interpolateDistance;
            var intenReal = intensity400 * averageWeighting + (1.0 - averageWeighting) * intensityWeighting;
            if (intenReal < 0.0)
                intenReal = 0.0;
            else if (intenReal >= 3.0) intenReal = 3.0;
            return intenReal;
        }
        ...
        private double ovalDistance(int x, int y, int width, int height)
        {
            var relX = x - width / 2;
            var relY = y - height / 2;
            var widthToHeightRatio = width / (double) height;


            return Math.Sqrt(relX * relX / widthToHeightRatio +
                             relY * relY * widthToHeightRatio);
        }
        ...
        private double getDistance(int x, int y, int width, int height)
        {
            ...
            return Math.Sqrt(relX * relX + (double) (relY * relY));
        }
        ...
        public static double getNearestAngel(int x, int y, int width, int height)
        {
            ...
            var angel = Math.Atan2(relY, relX) * 180.0 / Math.PI;
            while (angel < 0.0) angel += 360.0;
            angel %= 360.0;
    ...
    public class filter_butterworth
    {
        ...
        {
            var newOutput = a1 * newInput + a2 * inputHistory[0] + a3 * inputHistory[1] -
