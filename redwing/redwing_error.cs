...
namespace redwing
{
    public class redwing_error : MonoBehaviour
    {
        ...
        private void Start()
        {
            if (englishWarnings || englishLore || lore.isEnglish()) setupWarningEN();
        ...
        private static void setupWarningEN()
        {
            if (redwingProblemCode != 0)
            {
                angleint.addLanguageString(new language_string("General", "PROLOGUE_EXCERPT_04",
                    "Thank you for your cooperation and enjoy Redwing!", false, 100));
                angleint.addLanguageString(new language_string("General", "PROLOGUE_EXCERPT_AUTHOR",
                    "- Avenging Angle, on behalf of the Redwing team", false, 100));
            }

            if ((redwingProblemCode & 4) != 0)
            {
                angleint.addLanguageString(new language_string("General", "PROLOGUE_EXCERPT_01",
                    "ERROR: One of your mods is either out of date or missing!", false, 100));
                angleint.addLanguageString(new language_string("General", "PROLOGUE_EXCERPT_02",
                    "Redwing CANNOT RUN with your current mod configuration.", false, 100));
                angleint.addLanguageString(new language_string("General", "PROLOGUE_EXCERPT_03",
                    "Please read modlog.txt for more details!", false, 100));
            }
            else if ((redwingProblemCode & 1) != 0)
            {
                angleint.addLanguageString(new language_string("General", "PROLOGUE_EXCERPT_01",
                    "WARNING. PLEASE READ THE REDWING README BEFORE PLAYING.", false, 100));
                angleint.addLanguageString(new language_string("General", "PROLOGUE_EXCERPT_02",
                    "Also check out the global config file: 'Redwing.settings' located with your save files", false,
                    100));
                angleint.addLanguageString(new language_string("General", "PROLOGUE_EXCERPT_03",
                    "This message will disappear on relaunching the game! Please relaunch to see lore here.", false,
                    100));
            ...
            {
                angleint.addLanguageString(new language_string("General", "PROLOGUE_EXCERPT_01",
                    "WARNING. ERROR ENABLING HANDICAP: You cannot handicap yourself with Blackmoth installed.", false,
                    100));
                angleint.addLanguageString(new language_string("General", "PROLOGUE_EXCERPT_02",
                    "Blackmoth simply makes you too much of a God to be nerfed.", false, 100));
                angleint.addLanguageString(new language_string("General", "PROLOGUE_EXCERPT_03",
                    "The handicap has been disabled ingame but not in the settings file...", false, 100));
            }
        }
